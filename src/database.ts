...
export class DBClient {
   ...
   */
  async createTable(table: string, columns: Record<string, string>): Promise<Record<string, any>[]> {
   ...
   */
  async find(table: string, options: QueryOptions = {}): Promise<Record<string, any>[]> {
    // 确保数据库连接已建立
    const db = await this.ensureConnected();
    try {
   ...
   */
  async findOne(table: string, options: QueryOptions = {}): Promise<Record<string, any> | null> {
    // 确保数据库连接已建立
    const db = await this.ensureConnected();
    try {
   ...
   */
  async update(table: string, where: Record<string, any>, data: Record<string, any>): Promise<number> {
    // 确保数据库连接已建立
    const db = await this.ensureConnected();
    try {
        // 构建WHERE子句和对应的参数
